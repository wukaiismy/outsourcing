  ...
  optimization: {
    splitChunks: {
      cacheGroups: {
        ...
        app: {
          // test: /\.js$/,
          test: path.resolve(__dirname, "./src/pages/common/index.js"),
          chunks: "all", //表示显示块的范围，有三个可选值：initial(初始块)、async(按需加载块)、all(全部块)，默认为all;
          name: "app", //拆分出来块的名字(Chunk Names)，默认由块名和hash值自动生成；
          minChunks: 1,
          reuseExistingChunk: true,
          enforce: true
        }
...
for (var Key in entries) {
  var items = { _html: "", chunks: ["vendor", "app"] };
  items._html = Key;
  items.chunks.push(Key);
  htmlArray.push(items);
}
console.log(htmlArray);
// for (var Key in entries) {
//   module.exports.plugins.push(new htmlWebpackPlugin(getHtmlConfig(Key, Key)));
// }
// Object.keys(entries);
//自动生成html模板
